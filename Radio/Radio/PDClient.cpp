...
bool PDClient::sendMessage(uint8_t type, uint8_t* data, uint8_t datalen) {
  uint8_t messagelength = datalen+2; // type + data + random number
  uint8_t tosend[messagelength];

  tosend[messagelength-1] = random(255);
  Serial.print("Random int is 0x");
  Serial.println(tosend[messagelength-1], HEX);
