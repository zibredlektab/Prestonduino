...
uint8_t PDServer::getData(uint8_t datatype, char* databuf) {
  ...
  if (millis() > this->lastupdate + 5) {
    ...
    char* newlensname = mdr->getLensName();
    if (newlensname != this->fulllensname) {
      // mark lens name for update
      this->updatelensname = true;
      strncpy(this->fulllensname, newlensname, newlensname[0]);
